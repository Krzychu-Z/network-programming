...
int main(int argc, char *argv[])
{
	...
	char filter_exp[1024] = "tcp";	/* The filter expression */
	...
	for(;;){
		...
		struct ip6_hdrctl *typ6;

		hdr = (struct ethhdr *)buff;
		ipv6hdr = (struct ip6_hdr *)(buff+sizeof(struct ethhdr));
		thdr = (struct tcphdr *)(buff+sizeof(struct ethhdr)+sizeof(struct ip6_hdr));
		...
		if( ntohs (hdr->h_proto) == ETHERTYPE_IPV6 ) {

			 printf("IP src addr = %s\n", inet_ntop(AF_INET6, (struct sockaddr  *) &ipv6hdr->ip6_src, str1, sizeof(str1)));
			 printf("IP dst addr = %s\n", inet_ntop(AF_INET6, (struct sockaddr  *) &ipv6hdr->ip6_dst, str2, sizeof(str2)));
			...
			if( ipv6hdr->ip6_nxt == IPPROTO_TCP ){
				...
				memcpy(out,buff+sizeof(struct ethhdr)+sizeof(struct ip6_hdr)+sizeof(struct tcphdr), 
					(n-sizeof(struct ethhdr)+sizeof(struct ip6_hdr)+sizeof(struct tcphdr)) );
					
				int k=0;
				printf("DATA = ");
				for(k=0; k< (n-sizeof(struct ethhdr)+sizeof(struct ip6_hdr)+sizeof(struct tcphdr)); k++){
					if((isprint(out[k])))
						printf("%c",out[k] );
					else
						printf("-");		
